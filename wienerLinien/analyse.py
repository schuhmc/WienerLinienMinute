...
class apiData:
    ...
    def getAvailable(self, filter=True):
        ...
        """
        available = (
            self.df.groupby(["station", "line", "towards"])
            .size()
            .rename("count")
            .reset_index()
        )
        ...
        if filter:
            available = available.loc[
                ~available["towards"].str.match(
                    "(.*\*)|(.*_)|.*(FFP2-MASKEN)|(.*\!)|.*NÄCHSTER|.*FAHRTBEHINDERUNG|.*STÖRUNG"
                )
            ]
    ...
    def fetchResults(self, which, filter=True):
        ...
        """
        res_df = self.outer_train_info.loc[
            (self.outer_train_info["station"] == which[0])
            & (self.outer_train_info["line"] == which[1])
            & (self.outer_train_info["towards"] == which[2])
        ]
        res_df = res_df.iloc[-1][
            "vehicles_df"
        ]  # this is weird and may need double indexing on different python verisons or windows/linux differnces?
    ...
    def trackMany(self, which, depth=2, max_diff=15.0, multithreaded=4):
        """
        ...
        self.outer_train_info = pd.DataFrame(
            columns=["vehicles_df", "station", "line", "towards"]
        )
        ...
        if multithreaded:
            with mp.Pool(multithreaded) as p:
                r = list(
                    tqdm(
                        p.imap(wrapper, enumerate(which)),
                        total=len(which),
                        desc="Total Progress",
                    )
                )
            ...
            for p, l in enumerate(which):
                av = wrapper((p, l), True)
                self.outer_train_info = self.outer_train_info.append(
                    {"vehicles_df": av, "station": l[0], "line": l[1], "towards": l[2]},
                    ignore_index=True,
                )

    def track(self, which, depth=2, max_diff=15.0, progress=True, position=None):
        """
        ...
        for index, row in tqdm(
            ...
            if len(all_vehicles) == 0:
                for i, cntdwn in enumerate(row[4:].values):
                    all_vehicles = all_vehicles.append(
                        {
                            "vehicle": vehicle(row["time"], cntdwn, i),
                            "arrived": 0,
                            "active": 1,
                            "lastPos": i,
                            "trackStart": row["time"],
                ...
                if row["time"] - prev_row["time"] < max_diff:
                    ...
                    if row[0] > prev_cntwn[0]:
                        ...
                        for idx, v in all_vehicles.loc[
                            all_vehicles["active"] == 1
                        ].iterrows():
                            ...
                            if all_vehicles.at[idx, "lastPos"] < 0:
                                ...
                                ret = all_vehicles.at[idx, "vehicle"].trackTime(
                                    row["time"], row[4:].values
                                )
                                if not ret:
                    ...
                    logging.debug(
                        f"time differnce over threshold ({row['time'] - prev_row['time']}). closing all trains"
                    )
                    for idx, v in all_vehicles.loc[
                        all_vehicles["active"] == 1
                    ].iterrows():
        ...
        for i, r in tqdm(
            all_vehicles.iterrows(), total=len(all_vehicles), disable=True
        ):
            r["vehicle"].calculateDT()

        complete_res = pd.DataFrame(
            {"countdown": [], "start": [], "end": [], "complete": [], "hour": []}
        )
...
class vehicle:
    ...
    def __init__(self, firstSeen, cntdwn, position) -> None:
        ...
        )  # dataframe containing countdown value, start of countdown value and end of countdown value
        self.times = self.times.astype(
            {
                "countdown": int,
                "start": float,
                "end": float,
        ...
        self.arrived = (
            False  # to keep track if the vehicle has arrived yet or is still pending
        )
    ...
    def trackTime(self, time, cntwns):
        ...
        try:
            if (
                not self.times.at[self.times.index[-1], "countdown"]
                == cntwns[self.position]
            ):
                ...
                if (
                    self.times.at[self.times.index[-1], "countdown"]
                    < cntwns[self.position]
                ):
                ...
                self.times = self.times.append(
                    {
                        "countdown": cntwns[self.position],
                        "start": time,
                        "end": pd.NA,
                        "complete": False,
                        "warning": False,
            ...
            logging.debug(
                f"Exception {e} at time {time}. Setting train inactive and continuing."
            )
            return False
